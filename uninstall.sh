...
case "${1:-}" in
    --purge) PURGE=1 ;;
    --help|-h) echo "Kullanım / Usage: ./uninstall.sh [--purge]"; exit 0 ;;
    "") ;;
    *) echo "Bilinmeyen seçenek / Unknown option: $1" >&2; exit 2 ;;
esac
...
if [ "$PURGE" -eq 1 ]; then
    printf "Kuyruk, geçmiş, ayarlar ve loglar da silinsin mi / Also delete queue, history, settings, and logs? [e/y/H/N] "
    read -r answer
    case "$answer" in
        e|E|y|Y)
            rm -rf "$APP_DATA" "$CONFIG_BASE/video-indirici" "$STATE_BASE/video-indirici" "$CACHE_BASE/video-indirici"
            echo "Kullanıcı verileri silindi / User data deleted."
            ;;
        *) echo "Kullanıcı verileri korundu / User data preserved." ;;
    esac
fi

echo "Downloader kaldırıldı / uninstalled."
