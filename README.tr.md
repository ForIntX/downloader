...
Proje sürümü: **1.0.0-beta.3** (`1.0 Beta`)
...
En kolay kurulum için [GitHub Releases](https://github.com/ForIntX/downloader/releases)
sayfasından `Android.apk` dosyasını indirin, telefonda açın ve Android istediğinde
tarayıcıya/dosya yöneticisine kurulum izni verin. `Android.aab` yalnız Play
Console içindir ve doğrudan telefona kurulamaz.
...
Çoğu kullanıcı [GitHub Releases](https://github.com/ForIntX/downloader/releases)
sayfasından `Windows-Setup.exe` dosyasını indirip kurulum sihirbazını izlemelidir.
Kurulum istemeyenler için taşınabilir ZIP de sunulur. Uygulama kod imzalı olana
kadar Windows SmartScreen uyarısı gösterebilir.

## Linux kurulumu

Debian/Ubuntu'da [GitHub Releases](https://github.com/ForIntX/downloader/releases)
sayfasından `Linux.deb` dosyasını indirip sistemin yazılım kurucusuyla açın.
Kurulumdan sonra uygulama menüsündeki **Downloader** simgesini kullanın.

Fedora, Arch ve desteklenen diğer sistemlerde `Linux.tar.gz` dosyasını çıkarıp
şunları çalıştırın:
...
Bir sürüm etiketi gönderildiğinde Linux, Android ve Windows paketleri otomatik
oluşturulup tek bir GitHub Release altında yayınlanır. Aynı işlem GitHub Actions
üzerinden elle de başlatılabilir.
